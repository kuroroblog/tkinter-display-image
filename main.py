...
class Application(tk.Frame):
    ...
    def drawLabelPngImg(self):
        ...
        # 描画は、main関数内で展開される。 → ローカル変数だとmain関数実行時に画像情報が残っていない。 → グローバル変数を利用して、関数終了後も値参照できるようにしておく。
        ...
        pngImg = tk.PhotoImage(
            file="/path_to/xxx.png"
        )
    ...
    def drawCanvasPngImg(self):
        ...
        canvas.photo = tk.PhotoImage(
            file="/path_to/xxx.png")

        # create_imageについて : https://toolmania.info/post-13026/
        # 第一引数 : x座標位置
        # 第二引数 : y座標位置
        # image : 画像の設定
    ...
    def drawLabelJpgImg(self):
        ...
        # Tkinter 8.6以降でも、標準Tkinterではjpg対応できないため、PILを利用する。
        # 公式ドキュメント(画像) : https://docs.python.org/ja/3/library/tkinter.html#images
        # ファイルopenコマンドについて : https://carp.cc.it-hiroshima.ac.jp/~tateyama/Lecture/AppEx/PythonImagePIL.html
        # (画像のパスをお好みでご入力ください。)
        img = Image.open(
            '/path_to/xxx.jpg', 'r')
